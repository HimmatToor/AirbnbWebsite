...
function DEN() {
  ...
  // Denver city center
  const centerDEN = [39.7392, -104.9903]
  ...
  const getNeighborhoodCenter = () => {
    if (selectedNeighborhood === "All") return centerDEN

    const subset = listings.filter(
      l => l.neighbourhood?.trim() === selectedNeighborhood
    )

    if (subset.length === 0) return centerDEN
  ...
  return (
    <div className="den-container">
      ...
      {/* BACK BUTTON */}
      <div className="den-topbar">
        <button className="back-button" onClick={() => navigate('/cities')}>
          ← Back
        </button>
      </div>

      <h1 className="den-title">Denver</h1>
      ...
      {/* FILTER CARD */}
      <div className="den-filter-card">
        <div className="den-filter-grid">
