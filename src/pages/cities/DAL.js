...
function DAL() {
  ...
  // Dallas city center
  const centerDAL = [32.7767, -96.7970]
  ...
  const getNeighborhoodCenter = () => {
    if (selectedNeighborhood === "All") return centerDAL

    const subset = listings.filter(
      l => l.neighbourhood?.trim() === selectedNeighborhood
    )

    if (subset.length === 0) return centerDAL
  ...
  return (
    <div className="dal-container">
      ...
      {/* BACK BUTTON */}
      <div className="dal-topbar">
        <button className="back-button" onClick={() => navigate('/cities')}>
          ← Back
        </button>
      </div>

      <h1 className="dal-title">Dallas</h1>
      <p className="dal-subtitle">
      ...
      {/* FILTER CARD */}
      <div className="dal-filter-card">
        <div className="dal-filter-grid">
